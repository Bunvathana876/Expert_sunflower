...
export function OverviewPage(): React.JSX.Element {
  ...
  if (isLoading) {
    return (
      <div aria-busy="true" className="space-y-6 sf-glass-card bg-white/85 dark:bg-[#2A3420]/80 backdrop-blur-md border border-stone-200/80 dark:border-white/10 shadow-lg rounded-2xl p-6 sm:p-8">
    ...
    <div className="space-y-8">
      ...
      <section className="sf-glass-card bg-white/85 dark:bg-[#2A3420]/80 backdrop-blur-md p-6 space-y-4 border border-stone-200/80 dark:border-white/10 shadow-lg rounded-2xl">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">Past 30 Days</span>
        </div>

        {data.checks_trend_30d.length === 0 ? (
          <p className="text-xs text-gray-500 dark:text-gray-400 py-8 text-center">{t("admin.no_activity_yet")}</p>
        ) : (
          <div className="pt-4">
            <div className="flex items-end gap-1.5 h-36 border-b border-stone-200 dark:border-white/10 pb-2">
              {data.checks_trend_30d.map((d) => {
                ...
                return (
                  ...
                  >
                    {/* Tooltip on hover */}
                    <div className="absolute -top-8 bg-stone-900 text-white text-[0.65rem] py-0.5 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10 font-mono shadow-md border border-white/10">
                      {d.date}: {d.count} checks
                    </div>
                    <div
                      style={{ height: `${barHeight}%` }}
                      className="w-full max-w-[1.25rem] bg-gradient-to-t from-amber-500 to-amber-400 group-hover:from-amber-400 group-hover:to-amber-300 rounded-t-md transition-all duration-300 shadow-xs"
                    />
                  </div>
                );
              })}
            </div>
            <div className="flex justify-between text-[0.65rem] text-gray-500 dark:text-gray-400 pt-2 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="sf-glass-card bg-white/85 dark:bg-[#2A3420]/80 backdrop-blur-md p-6 space-y-4 border border-stone-200/80 dark:border-white/10 shadow-lg rounded-2xl">
          ...
          {data.top_symptoms.length === 0 ? (
            <p className="text-xs text-gray-500 dark:text-gray-400 py-4 text-center">{t("admin.no_symptoms_recorded")}</p>
          ) : (
            <div className="space-y-2">
              {data.top_symptoms.map((s, idx) => (
                <div
                  key={s.symptom_id}
                  className="flex items-center justify-between p-3 rounded-xl bg-stone-50/80 dark:bg-[#1E2615]/70 border border-stone-200/70 dark:border-white/10 text-xs shadow-xs"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span className="w-5 h-5 rounded-full bg-stone-200/80 dark:bg-stone-700/80 text-[0.65rem] font-bold flex items-center justify-center text-stone-700 dark:text-stone-200 shrink-0">
                      {idx + 1}
                    </span>
                  ...
                  <span className="px-2.5 py-0.5 rounded-full font-mono font-bold bg-amber-500/15 text-amber-800 dark:text-amber-300 text-[0.7rem] border border-amber-500/20">
                    {s.count} reports
                  </span>
                </div>
              ))}
        ...
        <section className="sf-glass-card bg-white/85 dark:bg-[#2A3420]/80 backdrop-blur-md p-6 space-y-4 border border-stone-200/80 dark:border-white/10 shadow-lg rounded-2xl">
          ...
          {data.no_match_patterns.length === 0 ? (
            <p className="text-xs text-gray-500 dark:text-gray-400 py-4 text-center">{t("admin.no_unmatched_patterns")}</p>
          ) : (
